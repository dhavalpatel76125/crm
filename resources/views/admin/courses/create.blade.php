...
<div class="w-full flex flex-col h-screen overflow-y-hidden">
    ...
    <div class="w-full overflow-x-hidden border-t flex flex-col">
        <main class="w-full flex-grow p-6">
            <h1 class="text-3xl text-black pb-6">Create new course</h1>
            {{-- // form for new course --}}
            ...
            <form action="{{ route('courses.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="max-w-lg mx-auto bg-white rounded-lg overflow-hidden shadow-lg p-6">
                    <div>
                        ...
                        <input type="text" name="name" class="h-12 px-3 w-full border-2 border-gray-300 rounded focus:outline-none focus:border-indigo-500" required>
                        @error('name')
                    ...
                    <div class="mt-4">
                        ...
                        <textarea name="description" class="h-32 px-3 w-full border-2 border-gray-300 rounded focus:outline-none focus:border-indigo-500" required></textarea>
                        @error('description')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mt-4">
                        ...
                        <input type="number" name="price" class="h-12 px-3 w-full border-2 border-gray-300 rounded focus:outline-none focus:border-indigo-500" required>
                        @error('price')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
